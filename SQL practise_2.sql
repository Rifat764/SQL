...
select * from parent2; --> Evet verileri sildi ve error vermedi

select * from child2; --> Fakat child2 tablosundaki bütün veriler de silindi (cascade marifeti)

--SORU2: parent2 tablosunu siliniz..

drop table parent2 cascade;  --> On delete cascade sayesinde child tablo silinmeden parent tablo silinebilir
...
-- SORU3: vergi_no'su 201 olan toptancinin sirket_ismi'ni 'nokia' , irtibat_ismi'ni 'Canan Can' olarak güncelleyiniz.
...
-- Not: Tabloları eski haline dönüştürelim..
...
update malzemeler set malzeme_isim = (x) where musteri_isim = 'Ali Can';
...
-- SORU3: meslekler tablosunda isim'i 'Ali' olanları, maas'a göre büyükten küçüge sıralayarak listeleyiniz

select * from meslekler where isim = 'Ali' order by maas desc;

-- SORU4: meslekler tablosunda soyisim 'i 'Bulut' olanları, maas'a gore sıralayarak listeleyiniz.
...
-- SORU1: sahislar tablosunda ulke'lere göre ortalama maas'ları listeleyiniz

select ulke, round(avg(maas)) as ortalama_maas from sahislar group by ulke;

-- SORU2: sahislar tablosunda ulke'lere göre kaç çalışan olduğunu listeleyiniz.

select ulke, count(ad) as calisan_sayisi from sahislar group by ulke;

-- SORU3: sahislar tablosundaki cinsiyet'e göre maas ortalamasını  listeleyiniz

select cinsiyet, round(avg(maas), 2) as maas_ort from sahislar group by cinsiyet;

-- SORU4: sahislar tablosunda ulke'lere göre ve sehir'lere göre çalışan sayısını listeleyiniz.
