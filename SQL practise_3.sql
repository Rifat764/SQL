...
/* -----------------------------------------------------------------------------
  SORU1: maas'i 4000’den cok olan personel isim'lerini ve maas'i 5000’den cok olan sehir'leri gosteren tekrarsız sorguyu yaziniz
------------------------------------------------------------------------------*/
...
/* -----------------------------------------------------------------------------
  SORU2: isim'i 'Mehmet Ozturk' olan kisilerin aldigi maas'lari  ve sehir'i 'Istabul' olan personelin maas'larini tekrarsız
...
/* -----------------------------------------------------------------------------
 ...
 'Honda' calisani olmayan personelin isim, maas, sirket bilgisini tekrarsız listeleyen bir sorgu yaziniz. 
------------------------------------------------------------------------------*/
...
-- select isim, maas, sirket from personel where maas < 5000
-- union
-- select isim, maas, sirket from personel where not sirket = 'Honda';  --->> Bu şekilde de olur
...
select sehir, maas, id from personel where id = '123456789'
union
...
-- union
-- select tel, cocuk_sayisi, id from personel_bilgi where id = '123' --->> İsim değişikliği olabilir
...
-- select film_name, category, actor_name from aktorler as B  right join filmler as A
-- on A.film_id = B.film_id;  --->> 2. Yol
...
-- on A.film_id= B.film_id  --->> aktorler ve filmler'e atama da yapılarak çözülebilir

-- select actor_name, film_name from filmler right join aktorler
-- on aktorler.film_id = filmler.film_id;   --->> 2. Yol
...
select A.ders_id, A.ders_ismi, A.ders_saati from qa_dersler as A inner join developer_dersler as B
on A.ders_id = B.ders_id;

-- select B.ders_id, B.ders_ismi, B.ders_saati from qa_dersler as A inner join developer_dersler as B
-- on A.ders_id = B.ders_id;   --->> Hangi kümenin ismini alacağımız önemli deği burada (B aldık)
...
select B.ders_id, B.ders_ismi, B.ders_saati from developer_dersler as A right join qa_dersler as B
on A.ders_id = B.ders_id;


select qa_dersler.ders_id,qa_dersler.ders_ismi,qa_dersler.ders_saati from qa_dersler left join developer_dersler
